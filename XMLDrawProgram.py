...
class TextCommand:
    def __init__(self,text,font="Helvetica",fontSize=16,color="black"):
        self.text = text
        self.font = font
    ...
    def draw(self, turtle):
        turtle.pencolor(self.color)
        turtle.write(self.text, False, align = "center",font =(self.font, int(self.fontSize), "normal"))
        
    def __str__(self):
        return '<Command text="' + str(self.text) + '" font="' + \
               str(self.font) + '" fontSize="' + self.fontSize + '" color="' + self.color + '">Text</Command>'
...
class DrawingApplication(tkinter.Frame):
    ...
    def buildWindow(self):
        ...
        def parse(filename):
            ...
            for commandElement in graphicsCommands:
                ...
                if command == "GoTo":
                    ...
                    fontSize = float(attr["fontSize"].value)
                    color = attr["color"].value.strip()
                    cmd = TextCommand(text,font,fontSize,color)
        ...
        textString = tkinter.Label(sideBar, text = "Text String")
        textString.pack()
        text = tkinter.StringVar()
        textEntry = tkinter.Entry(sideBar, textvariable = text)
        text.set(str("I lost the Game"))
        textEntry.pack()
        
        fontLabel = tkinter.Label(sideBar, text = "Font")
        fontLabel.pack()
        font = tkinter.StringVar()
        fontEntry = tkinter.Entry(sideBar, textvariable = font)
        font.set(str("Helvetica"))
        fontEntry.pack()
        
        fontSizeLabel = tkinter.Label(sideBar, text = "Font Size")
        fontSizeLabel.pack()
        fontSize = tkinter.StringVar()
        fontSizeEntry = tkinter.Entry(sideBar, textvariable = fontSize)
        fontSize.set(5)
        fontSizeEntry.pack()

        def textHandler():
            cmd = TextCommand(str(text.get()), str(font.get()), float(fontSize.get()), penColor.get())
            cmd.draw(theTurtle)
            self.graphicsCommands.append(cmd)
